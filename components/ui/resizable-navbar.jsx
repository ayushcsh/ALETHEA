  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "bg-[#12121280] dark:bg-[#12121280]", // ALWAYS SEMI-TRANSPARENT
        className
      )}
    >
      {children}
    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-5 py-2 w-fit text-white dark:text-neutral-300 hover:text-black"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-[#0a0a0a] dark:bg-neutral-950/80",
        className
      )}
    >
      {children}
...
export const MobileNavHeader = ({ children, className }) => {
  return (
    <div
      className={cn(
        "flex w-full mt-[-25px] flex-row items-center justify-between",
        className
      )}
    >
      {children}
...
export const NavbarLogo = () => {
  return (
    <a
      href="#"
      className="relative z-20 mr-4 flex items-center space-x-2 px-2 py-1 text-sm font-normal text-black translate-y-[-4.5px]"
    >
      <Image
        src="/images/LETHEA.png"
        alt="Logo"
        width={130}
        height={60}
        className=" w-[80px]    h-[35px]
          md:w-[130px] md:h-[35px] "
      />
    </a>
  );
};
