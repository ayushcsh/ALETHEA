...
const InputQuery = ({ chatId, showNotepad, showPDF }) => {
  ...
  const pdfId = typeof window !== "undefined" ? localStorage.getItem("pdfId") : null;
  ...
  useEffect(() => {
    ...
    const formatted = prevmessage.map((msg) => {
      ...
      return {
        ...
        sources: ["assistant", "ai", "bot"].includes(role) ? msg.sources || [] : [],
        isLoading: false,
        type: "text",
      };
    });
  ...
  const handlesend = async () => {
    ...
    try {
      await inputAns({ chatId, userId, message: userquery, pdfId });
    } catch {
      setChat((prev) => [
        ...prev,
        {
                  ...
                  <div className="relative max-w-[98%] w-fit rounded-[15px] p-2 font-[Arial] mb-6">
                    {item.isLoading ? (
                                ...
                                <div className="group relative h-64 [transform-style:preserve-3d] transition-transform duration-500 hover:rotate-y-180">
                                  <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-primary p-6 shadow-md backface-hidden [backface-visibility:hidden] group-hover:opacity-0 transition-opacity duration-300">
                                    <div className="text-center">
                                      ...
                                      <p className="mt-2 text-[14px] md:text-[17px] font-[Arial] text-white">{card.question}</p>
                                    </div>
                                  </div>
                                  <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-white p-6 shadow-md backface-hidden [transform:rotateY(180deg)] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <div className="text-center">
                                      ...
                                      <p className="mt-2 text-[14px] md:text-[17px] font-[Arial] text-gray-900">{card.answer}</p>
                                    </div>
                                  </div>
                                </div>
                              </CarouselItem>
                      ...
                      <div className="relative">
                        ...
                        <div className="flex items-center mb-3 mt-1 ml-2 relative">
                          {/* Copy */}
                          <button onClick={() => handleCopy(item.answer, index)} className="text-white hover:bg-[#262626] rounded-[15px] p-2 flex items-center gap-1">
                            {copied === index ? <FaCheck className="text-green-400 md:h-4 md:w-4 h-4 w-4" /> : <BiCopy className="md:h-4 md:w-4 h-3 w-3" />}
                          </button>

                          {/* Share */}
                          <button onClick={() => setShareOpen(shareOpen === index ? null : index)} title="Share Response" className="text-white hover:bg-[#262626] rounded-[15px] p-2 flex items-center gap-1">
                            <FaRegShareFromSquare className="md:h-4 md:w-4 h-3 w-3" />
                          </button>

                          {/* ✅ Download Button */}
                          <button onClick={() => handledownload(item.query, item.answer, index)} title="Download Response" className="text-white hover:bg-[#262626] rounded-[15px] p-2 flex items-center gap-1">
                            <MdOutlineFileDownload className="md:h-4.5 md:w-4.5 h-3 w-3" />
                          </button>

                          {/* Sources — filenames reveal to the right on hover */}
                          {sources.length > 0 && (
                            <div className="group flex items-center ml-1">
                              <button className="flex items-center gap-1.5 rounded-[15px] p-2 text-gray-300 hover:bg-[#262626] hover:text-white">
                                <BookOpen className="md:h-4 md:w-4 h-3 w-3" />
                                <span className="text-[13px] font-[Arial]">Sources</span>
                              </button>
                              <div className="flex items-center gap-1.5 max-w-0 opacity-0 overflow-hidden group-hover:max-w-[420px] group-hover:opacity-100 transition-all duration-300 ease-out">
                                {sources.map((s, si) => (
                                  <span
                                    key={`${index}-source-${si}`}
                                    title={s}
                                    className="inline-flex items-center gap-1.5 text-[12px] text-gray-300 bg-[#1c1c1c] border border-[#2b2b2b] rounded-full pl-2 pr-3 py-1 font-[Arial] truncate max-w-[180px] ml-1.5 shrink-0"
                                  >
                          ...
                          <AnimatePresence>
                            {shareOpen === index && (
                              <motion.div ref={(el) => (shareRefs.current[index] = el)} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.25 }} className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-[200]" onClick={() => setShareOpen(null)}>
                                <div onClick={(e) => e.stopPropagation()} className="pointer-events-auto">
                                  <BackgroundGradient animate={true} containerClassName="rounded-2xl p-[2px] w-[560px] max-w-[90%] shadow-2xl" className="bg-[#1c1c1c] rounded-[24px] h-[80vh] w-full p-6 border border-[#1f1f1f] text-white">
                                    <div>
                                      ...
                                      <div className="w-[450px] h-[550px] mt-[20px] flex flex-col items-center">
                                        ...
                                        <div className=" text-center">
                                          ...
                                          <div className="flex justify-center items-center gap-8">
                                            <button className="p-3 rounded-full bg-[#FFA500]/10 border border-[#FFA500]/40 hover:bg-[#FFA500]/20 text-[#FFA500] transition-all duration-300 hover:scale-105" onClick={() => { const messageLink = `${window.location.origin}/chat/${chatId}?message=${index}`; navigator.clipboard.writeText(messageLink); setShareOpen(null); }}>
                                              <FiLink className="w-5 h-5" />
                                            </button>

                                            <button className="p-3 rounded-full bg-[#1DA1F2]/10 border border-[#1DA1F2]/40 hover:bg-[#1DA1F2]/20 text-[#1DA1F2] transition-all duration-300 hover:scale-105" onClick={() => { window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(item.answer)}`, "_blank"); setShareOpen(null); }}>
                                              <FaTwitter className="w-5 h-5" />
                                            </button>

                                            <button className="p-3 rounded-full bg-[#0077b5]/10 border border-[#0077b5]/40 hover:bg-[#0077b5]/20 text-[#0077b5] transition-all duration-300 hover:scale-105" onClick={() => { window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}&summary=${encodeURIComponent(item.answer)}`, "_blank"); setShareOpen(null); }}>
                                              <FaLinkedinIn className="w-5 h-5" />
                                            </button>

                                            <button className="p-3 rounded-full bg-[#FF4500]/10 border border-[#FF4500]/40 hover:bg-[#FF4500]/20 text-[#FF4500] transition-all duration-300 hover:scale-105" onClick={() => { window.open(`https://reddit.com/submit?url=${encodeURIComponent(window.location.href)}&title=${encodeURIComponent(item.answer)}`, "_blank"); setShareOpen(null); }}>
