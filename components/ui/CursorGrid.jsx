...
// Manual rounded-rect path (avoids relying on ctx.roundRect, which isn't
// available in every browser/canvas polyfill and would otherwise throw
// mid-frame and silently kill the whole animation loop).
  ...
  useEffect(() => {
    ...
    const draw = now => {
      ...
      try {
        ...
        // A single bad frame should never permanently blank the canvas —
        // log it and keep the loop alive instead of dying silently.
        console.error('CursorGrid draw error:', err);
        raf = requestAnimationFrame(draw);
      }
    };

    const drawFrame = (p, now, dt) => {
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        ctx.beginPath();
        if (p.cellRadius > 0) {
          roundedRectPath(ctx, x, y, s, s, p.cellRadius);
        } else {
          ctx.rect(x, y, s, s);
        }
        if (p.fillOpacity > 0) {
    ...
    // Listen on window (not just the container) so the grid still reacts
    // when the pointer is over content stacked above it — those elements
    // intercept the hit-test, but the event still bubbles up to window.
